...
#+BEGIN_SRC emacs-lisp
  (defmacro f! (str) `(s-lex-format ,str))
#+END_SRC
    ...
    (let* ((byte-compile-warnings '(not unresolved free-vars))
      ...
      (when (or (called-interactively-p 'interactive)
        ...
        (condition-case e
          ...
          (error (shell-command (f! "notify-send 'ERROR' '${e}'")))))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defconst std::ledger::dir (expand-file-name (f! "${*ORGDIR*}/Ledger")))
...
#+BEGIN_SRC emacs-lisp
  (defun std::ledger ()
    (interactive)
    (require 'calendar)
    (-let [pcmds (f! "${*ORGDIR*}/Ledger/ledger.el")]
      (when (file-exists-p pcmds)
        (load-file pcmds)))
    (let* ((date (calendar-current-date))
           (year (cl-third (calendar-current-date)))
           (main-file (f! "${std::ledger::dir}/Ledger.ledger"))
           (year-file (f! "${std::ledger::dir}/${year}.ledger")))
...
#+BEGIN_SRC emacs-lisp
  (defun std::ledger::mode-hook ()
    ...
    (evil-ledger-mode)
    (set-local!
     ledger-accounts-file     (f! "${std::ledger::dir}/Ledger.ledger")
    ...
    (defun std::ledger::goto-current-month ()
      ...
      (-let [month (-> (calendar-current-date)
        ...
        (save-match-data
          (-let [start (point)]
            (goto-char 0)
            (if (search-forward (f! "+++ ${month}") nil :no-error)
                (forward-line 1)
...
#+BEGIN_SRC emacs-lisp
  (after! mu4e
    ...
    (evil-keybind! 'evilified (mu4e-headers-mode-map mu4e-view-mode-map mu4e-conversation-map)
      "J" #'std::quick-forward
      "K" #'std::quick-backward))
#+END_SRC
        ...
        (-let [entry (if (eq major-mode 'elfeed-show-mode) elfeed-show-entry (elfeed-search-selected :single))]
          (if (s-matches? (rx "https://www.youtube.com/watch" (1+ any)) (elfeed-entry-link entry))
              (let* ((quality (completing-read "Max height resolution (0 for unlimited): " '("0" "480" "720" "1080")))
                     (arg (if (= 0 (string-to-number quality)) "" (f! "--ytdl-format=[height<=?${quality}]"))))
...
#+BEGIN_SRC emacs-lisp
  (after! elfeed
    (setf elfeed-db-directory  (f! "${*ORGDIR*}/Elfeed-DB")
...
#+BEGIN_SRC emacs-lisp
  (after! make-mode
    ...
    (defun std::make::mode-hook ()
      (setf company-backends '((comany-capf company-dabbrev-code :with company-yasnippet))))
...
More accessible avy
#+BEGIN_SRC emacs-lisp
  (evil-keybind! (normal motion visual) global-map
