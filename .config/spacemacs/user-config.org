...
#+BEGIN_SRC emacs-lisp
  (defmacro keybind! (keymap &rest keybinds)
    (declare (indent 1))
    (cl-assert (= 0 (% (length keybinds) 2)) "Uneven number of keybinds!")
    (-let [bind-forms nil]
      (while keybinds
        (-let [(key func . rest) keybinds]
          (push `(define-key ,keymap (kbd ,key) ,func) bind-forms)
          (setq keybinds rest)))
...
#+BEGIN_SRC emacs-lisp
  (defun std::dired::filesize ()
    ...
    (let* ((file (dired-filename-at-point))
           (cmd (if (f-directory? file) "du -sh %s" "ls -sh %s"))
           (output (->> file
...
~std::dired::goto-videos~
#+BEGIN_SRC emacs-lisp
(defun std::dired::goto-variables ()
  (interactive)
  (dired "~/Videos/"))
#+END_SRC
...
~std::ledger::window-setup~
#+BEGIN_SRC emacs-lisp
  (defun std::ledger::window-setup ()
    (interactive)
    (require 'calendar)
    (let* ((months '((1 . "Januar")
                     (2 . "Februar")
           ...
           (year (caddr (calendar-current-date)))
           (month-num (car date))
           (month (alist-get month-num months))
           (path-template "~/Documents/Org/Ledger/%s/%s.ledger")
           (path (format path-template year month)))
      (unless (file-exists-p path)
        (if (/= 1 month-num)
            (setq path (format path-template
                               year (alist-get (1- month-num) months)))
          (setq year (1- year)
                path (format path-template
                             year (alist-get 12 months)) )))
      (delete-other-windows)
      (find-file path)
      (split-window-right)
      (other-window 1)
      (find-file "~/Documents/Org/Ledger/Ledger.ledger")))
#+END_SRC

~std::ledger::set-accounts-file~
#+BEGIN_SRC emacs-lisp
  (after! ledger-mode
    (defun std::ledger::set-accounts-file ()
      (when (s-starts-with? (f-expand "~/Documents/Org/Ledger") (buffer-file-name))
        (setq-local ledger-accounts-file "~/Documents/Org/Ledger/Ledger.ledger"))))
#+END_SRC
...
Set accounts file only in proper files
#+BEGIN_SRC emacs-lisp
  (add-hook 'ledger-mode-hook #'std::ledger::set-accounts-file)
#+END_SRC
    ...
    (defun std::mail::find-thunderbird-imap-dir ()
      (-let [dot-thunderbird (expand-file-name "~/.thunderbird")]
        (when (f-exists? dot-thunderbird)
          (-when-let (maildir (--find (member "ImapMail" (directory-files it))
...
#+BEGIN_SRC emacs-lisp
  (-let [framey-dir (expand-file-name "~/Documents/git/framey")]
    (when (f-exists? framey-dir)
      (use-package framey
        :load-path "~/Documents/git/framey"
        :defer t)
      (autoload #'framey-mode "framey")
