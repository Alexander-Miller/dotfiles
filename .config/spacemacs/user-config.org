...
#+BEGIN_SRC emacs-lisp
  (add-hook 'kill-emacs-hook #'std::setup-config-before-quit)
...
do it manually.
#+BEGIN_SRC emacs-lisp
  (defun std::elisp-repl (&optional arg)
    (interactive "P")
...
#+BEGIN_SRC emacs-lisp
  (with-eval-after-load 'company
    (defconst std::backend-priorities
      '((company-anaconda     . 0)
        (company-capf         . 6)
        (company-yasnippet    . 7)
        (company-keywords     . 8)
    ...
    (defun std::priority-of-backend (backend)
  ...
  Hence only the less important backends neet to be explicitly marked."
      (let ((pr (cdr (assoc backend std::backend-priorities))))
        (if (null pr) -1 pr)))
      ...
      (sort
       ...
       (lambda (c1 c2)
         (let* ((b1 (get-text-property 0 'company-backend c1))
                ...
                (diff (- (std::priority-of-backend b1) (std::priority-of-backend b2))))
           (if (= diff 0)
               nil
             (if (< 0 diff) nil t)))))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun std::use-completions-priority-sorting ()
  ...
  (--each '(rust-mode-hook fish-mode-hook python-mode-hook)
    (add-hook it #'std::use-completions-priority-sorting))
#+END_SRC
