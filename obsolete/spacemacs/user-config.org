...
#+BEGIN_SRC emacs-lisp :tangle autoloads.el
  (defun std::edit-org-user-config ()
    ...
    (interactive)
    (find-file-existing (concat std::spacemacsdir "/user-config.org")))
#+END_SRC
...
Helper macro to wrap a dired command to enable use of ~dired-dwim-target~ with a prefix arg.
#+BEGIN_SRC emacs-lisp
...
For the sake of clarity the dired-mode-hook wipes out dired's keymaps. All we have is defined here.
#+BEGIN_SRC emacs-lisp
  ...
  (std::after dired+
    ...
    (std::keybind evil-dired-state-map
      "q"   #'std::dired::quit
      "RET" #'dired-find-file;;TODO(2020/03/16): find file close dired
      "l"   #'dired-find-file
...
#+BEGIN_SRC emacs-lisp :tangle autoloads.el
  (defun std::ledger ()
    (interactive)
    (require 'calendar)
    (-let [pcmds (std::fmt "${std::orgdir}/Ledger/ledger.el")]
